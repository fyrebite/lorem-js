...
function lorem(wordcount){
	...
	//make sure there are enough words left in the source
	var max = words.length - wordcount;
	if(random > max){
		lorem(wordcount);
	}

	//pick a random sentence to start from
	var start = sentences[random];

	//find out the word position of the start sentence
	var pos = words.indexOf(start);

	//starting from the random position, get the amount of words requested
	var output = [];
	for(var i = pos; i < pos + wordcount; i++){
		output.push(words[i]);
	}
